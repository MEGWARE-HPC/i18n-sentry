...
export function extractI18nKeys(source: string): { key: string; line: number }[] {
    ...
    //    [translate]="'key'"
    //    [translate]="\"key\""
    const directiveBindingRe = /\[translate\]\s*=\s*["']\s*(['"])([^'"]+)\1\s*["']/g;
    ...
    //    .stream('key')
    //    .translate('key')
    const serviceRe = /\.(?:instant|get|stream|translate)\(\s*(['"])([^'"]+)\1/g;
    ...
    //    translate('key')
    const standaloneFnRe = /(?<![.\w$])translate\(\s*(['"])([^'"]+)\1/g;
    ...
    const arrayMethodRe = /\.(?:instant|get|stream|translate)\(\s*\[([\s\S]*?)\]/g;
...
export function extractRawTextNodes(source: string, config: SentryConfig): FileIssue[] {
    ...
    while ((m = re.exec(cleaned)) !== null) {
        ...
        if (/^[{}\s]*@(if|else|for|switch|case|default|empty|defer|loading|placeholder|error|let)\b/.test(text)) {
        continue;
        }
