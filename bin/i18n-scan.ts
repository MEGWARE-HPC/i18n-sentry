...
import type { Extractor, FileIssue } from "../core/config.js";
...
import { collectFiles, readFile } from "../utils/files.js";
...
function buildAttributeSets(config: ReturnType<typeof loadConfig>) {
    ...
    const warnAttrs = new Set(["aria-label", "aria-description", "title", ...config.warnAttributes]);
...
function main() {
    ...
    printSimpleSection(`Unused keys (${summary.unusedKeys.length})`, summary.unusedKeys, red, ({ key }) => red(key));
