...
import type { LocaleMap, SentryConfig } from "./config.js";
    ...
    for (const locale of config.locales) {
        if (structure === "nested") {
            ...
            if (!existsSync(dirPath)) {
                console.error(`\x1b[31mX  Locale directory not found: ${dirPath}\x1b[0m`);
                process.exit(1);
            }
            ...
            if (!existsSync(filePath)) {
                console.error(`\x1b[31mX  Locale file not found: ${filePath}\x1b[0m`);
                process.exit(1);
            }
