...
export interface SentryConfig {
  ...
  /**
   * AST-Mode (Vue <script setup>): welche dot-path Function Calls gescannt
   * werden, und welche 0-basierten Argument-Indizes davon Text-Kandidaten sind.
   * "all" scannt jedes string-literal Argument.
   * z.B. { "toast.success": "all", "createTextColumn": [2] }
   */
  scriptScanFunctions: Record<string, "all" | number[]>;
  /**
   * AST-Mode: Property-Keys in Objekt-Literalen deren String-Value gescannt
   * wird, z.B. { label: "Offen" } bei scriptScanProperties: ["label"]
   */
  scriptScanProperties: string[];
}
...
export function loadConfig(): SentryConfig {
  ...
  return {
    ...
    scriptScanFunctions: raw.scriptScanFunctions ?? {},
    scriptScanProperties: raw.scriptScanProperties ?? [],
  };
}
